...
interface BlackSheep40KProps {
  products: Product[];
  hideHeader?: boolean;
}

const BlackSheep40K = ({ products, hideHeader = false }: BlackSheep40KProps) => {
    ...
    <section className={`w-full ${hideHeader ? '' : 'bg-white rounded-lg shadow-lg overflow-hidden border border-gray-200 mb-8'}`}>
      {!hideHeader && (
        <>
          ...
          <h2 className="text-2xl md:text-3xl font-bold">BLACK SHEEP 40K</h2>
