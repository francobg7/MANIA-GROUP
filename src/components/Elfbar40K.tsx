...
interface Elfbar40KProps {
  products: Product[];
  hideHeader?: boolean;
}

// Componente para mostrar una subcategoría dentro de Elfbar40K
const Elfbar40KSubcategory = ({ 
  title, 
  products, 
  hideHeader 
}: { 
...
const Elfbar40K = ({ products, hideHeader = false }: Elfbar40KProps) => {
  // Separar productos por subcategoría
  const iceKingProducts = products.filter((p) => p.name.includes("ICE KING"));
  const nicotKingProducts = products.filter((p) => p.name.includes("NICOT KING"));
  const sourKingProducts = products.filter((p) => p.name.includes("SOUR KING") || p.name.includes("SWEET KING"));
  const trioProducts = products.filter((p) => p.name.includes("TRIO"));
  ...
  return (
    <section className={`w-full ${hideHeader ? '' : 'bg-white rounded-lg shadow-lg overflow-hidden border border-gray-200 mb-8'}`}>
      {!hideHeader && (
        <>
          ...
          <div className="bg-gradient-to-r from-gray-800 to-gray-900 text-white px-6 py-4">
            <h2 className="text-2xl md:text-3xl font-bold">ELFBAR 40K</h2>
      ...
      {/* Contenido: Las 4 subcategorías */}
      <div className="space-y-0">
        {iceKingProducts.length > 0 && (
          <div className="border-t border-gray-200">
            <Elfbar40KSubcategory title="ELFBAR 40K ICE KING" products={iceKingProducts} hideHeader={hideHeader} />
          </div>
        )}
        {nicotKingProducts.length > 0 && (
          <div className="border-t border-gray-200">
            <Elfbar40KSubcategory title="ELFBAR 40K NICOT KING" products={nicotKingProducts} hideHeader={hideHeader} />
          </div>
        )}
        {sourKingProducts.length > 0 && (
          <div className="border-t border-gray-200">
            <Elfbar40KSubcategory title="ELFBAR 40K SOUR KING" products={sourKingProducts} hideHeader={hideHeader} />
          </div>
        )}
        {trioProducts.length > 0 && (
          <div className="border-t border-gray-200">
            <Elfbar40KSubcategory title="ELFBAR 40K TRIO" products={trioProducts} hideHeader={hideHeader} />
          </div>
        )}
      </div>
    </section>
